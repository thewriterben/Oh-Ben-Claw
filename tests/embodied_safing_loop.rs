...
async fn battery_drain_engages_then_recovers_shed_load() {
    ...
    assert!(
        !state.shed_load(),
        "shed_load auto-recovers once charge returns"
    );
}
...
async fn network_loss_engages_then_recovers_net_safing() {
    ...
    ctl.tick_and_dispatch(1_000).await.unwrap();
    assert!(
        !state.offline() && !state.degraded_net(),
        "online: no net safing"
    );
...
async fn independent_subsystems_do_not_cross_clear() {
    ...
    power
        .ingest(
            &battery(12.0, ChargeState::Discharging),
            1_000,
            oh_ben_claw::memory::world::Origin::Observed,
        )
        .unwrap();
    ...
    assert!(
        state.shed_load(),
        "power shed persists while battery is low"
    );
