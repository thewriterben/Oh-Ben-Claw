...
Checkbox markers inside a phase:
- `[x]` — done
- `[ ]` — not done
- `[~]` — **written but not wired**: the code exists and nothing calls it. Added
  2026-07-29 after an audit found a box checked for authorship rather than for
  reachability. A `[~]` is a bug in this document as much as in the code.
- `[-]` — **removed**, with the date and the reason. Deleted work is struck
  through rather than erased, so that a reader who remembers a feature can find
...
Generated by `scripts/subsystem_ledger.py`. Re-run it rather than editing the
table; a hand-maintained inventory is the thing that produced the problem it
exists to fix.

### Why this is here

The phases below describe how the system was *built*. They turned out not to
describe what it *is*. Five modules had no row anywhere in this document —
`foresight`, `power`, `gnss`, `comms`, `aerial`, 1,826 LOC — and `foresight`
carries a whole section in `docs/SOTA-COMPARISON.md` while having no phase to
carry its status. Work with a public claim and no row is invisible to the process
meant to gate it.

That matters now because development migrates to Open Body Control piecewise, as
each piece becomes defensible. That policy needs a list of pieces and, per piece,
what evidence exists. This is that list.

Two columns are the point:

- **claim** — `benchmarked` if `docs/SOTA-COMPARISON.md` compares it against the
  state of the art, else `documented`. Derived from a declared
  section-to-module mapping in the script, not from counting words: a first pass
  counted names and flagged `cost`, `runtime` and `learning`, because "compute
  cost", "runtime shields" and "machine learning" appear there as prose. Three of
  four gaps were artifacts of the measurement.
- **evidence** — `unwired` (nothing outside the module references it) <
  `none` < `unit` < `integration` (an integration suite exercises it) <
  `bench` (validated against real hardware or a live store; **declared** in the
  script with a citation, because no static analysis can see it).

A ⚠ marks a claim outrunning its evidence. That is not an accusation that the
code is wrong — it means the module is un-*assessed*, which is precisely what
blocks it from migrating.
...
| `a2a` | 871 | 18 | 1 | `evals` | 11 | documented | integration |
| `movement` | 741 | 14 | 11 | `embodied_full_stack`, `embodied_hil_loop`, `system2_eval` | 4 | documented | integration |
| `siteplan` | 610 | 15 | 4 | `planner_parity` | 8 | documented | integration |
| `harness` | 577 | 3 | 2 | `harness_long_horizon` | 8 | documented | integration |
...
- **`power`** and **`comms`** — both wired and both covered by
  `embodied_safing_loop`, so the evidence is fine; they simply have no row. Give
  them one.
- **`gnss`** and **`aerial`** (336 + 177 LOC) — no phase, no README mention, unit
  tests only, 1 and 2 external references. These are the genuinely un-assessed
  modules in the tree. Either they are early Conservation-Grid G2+ work that
  should say so, or they are speculative and should say that instead.

### The blind spot below the ledger

The table above works at module granularity, and that is not fine enough. Two of
the three worst overclaims in this tree were **files inside live modules**, which
a module-level survey cannot see by construction:

- `memory/personality.rs` — SOUL.md / USER.md. Documented in this file and in the
  README, never called. Found by hand 2026-07-28, removed.
...
`security/pairing.rs` does **not** appear in the file sweep, correctly.
`NodePairingManager` is referenced — it is a `SecurityManager` field, constructed
at startup. What never happens is anyone asking it anything: `pair_node` zero
callers, `is_trusted` zero, the field never read, `require_pairing` validated and
then ignored.
...
| type constructed and never interrogated | **nothing — read the code** |

The third needs method-level call-graph analysis. It is also the most dangerous of
the three, because the code is reachable, instantiated, covered by unit tests, and
inert. `pairing.rs` has 386 lines and its own passing tests. A config key turns it
on. It authenticates nothing.

### What the ledger corrected

An earlier assessment of this tree claimed navigation, SLAM, fleet coordination
and foresight had "no phase, no eval and no bench evidence". Measured: `navigation`
has 55 test functions and is exercised by three integration suites
(`embodied_full_stack`, `embodied_hil_loop`, `spine_fleet_e2e`); `fleet` has three
(`mesh_fleet_e2e`, `offgrid_fleet_loop`, `spine_fleet_e2e`); `mission` and
`movement` likewise. Only `foresight` was unit-tested-only. The claim was made
from a word-frequency reading of this file and was wrong about three of its four
subjects. Hence a script, and hence the instruction at the top not to maintain the
table by hand.
...
- [~] Node pairing — HMAC-SHA256 tokens, 5-minute replay window, quarantine status (`src/security/pairing.rs`) — *(audit 2026-07-30: implemented and constructed, never consulted. `pair_node` and `is_trusted` have zero callers; the `SecurityManager.pairing` field is never read; `require_pairing` is validated but enforces nothing. Nodes are not authenticated. This is the base `src/security/trust.rs` assumes in its header and does not have.)*
...
- [-] **`PersonalityStore`** — `SOUL.md` / `USER.md` *(**removed** 2026-07-28: implemented and documented, never called from `build_context()`. Set `[agent].system_prompt` instead. If standing agent identity is wanted back, it is a new decision, not a restoration.)*
- [-] `build_system_prompt()` helper *(**removed** 2026-07-28 with `PersonalityStore`)*
...
- [x] `Config` gains `proxy: ProxyConfig` *(`personality: PersonalityConfig` **removed** 2026-07-28)*
- [x] `FeishuConfig`, `ProxyConfig` structs in `src/config/mod.rs` *(`PersonalityConfig` **removed** 2026-07-28)*
