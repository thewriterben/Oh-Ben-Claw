...
- [x] **Feed the codegen:** `SitePlan::to_toml` emits a paste-ready `[site]` + `[[site.node]]` section (id + geodetic + ENU per node), TOML-validated, that merges alongside a deployment config; `SitePlan::summary` for operator display. *Follow-up (deployment owner): attach positions directly onto `DeploymentScheme` hardware in `scheme.rs`.*
