...
| **G7** — Orbital imagery | 🟡 **Analysis core done** | ClawCam `analytics.habitat` — `LandCover` raster + `build_habitat_report`: per land-cover class, detection **use** vs area **availability** with selection ratio + Ivlev electivity (the habitat-preference analysis satellite classification enables). Pure builder. *Remaining: the actual Sentinel/Landsat tile fetch + classification into a stored `land_cover` layer, then tool/REST wiring.* |
