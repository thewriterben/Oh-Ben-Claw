...
//! GPS module joins the same coordinate frame as the drones and cameras. An optional
//! `origin` sets the frame anchor (default: the fix's own position, node at 0,0).
//!
...
impl Tool for GnssFixTool {
    ...
    fn description(&self) -> &str {
        "Decode a raw NMEA GGA sentence from a GPS/GNSS receiver into a position and a fleet \
         ...
         ([lat, lon]) anchors the local ENU frame (default: the fix itself), `node_id`, and \
         `now_ms`. Returns the decoded fix (lat/lon/alt, quality, satellites, hdop, has_fix) \
         and a fleet node_state (local x/y metres). Pure computation — no approval."
    }
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        ...
        // Frame origin: explicit [lat, lon] if given, else the fix's own position.
        let frame = match args.get("origin").and_then(Value::as_array) {
            Some(o) => {
                let lat = o.first().and_then(Value::as_f64);
                let lon = o.get(1).and_then(Value::as_f64);
                match (lat, lon) {
                    (Some(lat), Some(lon)) => GeoFrame::new(GeoPoint::new(lat, lon, 0.0)),
                    _ => return Ok(ToolResult::err("'origin' must be [lat, lon]")),
                }
            }
            None => GeoFrame::new(fix.to_geopoint()),
        };
        let node_id = args.get("node_id").and_then(Value::as_str).unwrap_or("gnss");
...
mod tests {
    ...
    async fn decodes_a_fix_and_projects_a_node() {
        let r = GnssFixTool::new().execute(json!({ "sentence": GGA })).await.unwrap();
