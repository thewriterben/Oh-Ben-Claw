...
mod tests {
    ...
    // Two tests lived here until 2026-08-13:
    //   a_reported_reading_is_an_assertion_and_cannot_drive_safing
    //   a_measured_reading_still_drives_safing
    // They are in tests/reflex_boundary.rs now. They named the agent's safing
    // module, and they were the *only* thing in this 8886-line
    // module that named `agent` at all -- a back-edge that put four cycles in
    // the dependency graph, held open by two test functions. The claim they
    // make spans the tool layer and the safing layer, so it belongs somewhere
    // that can name both without either owning the other.
    //
    // Written without spelling the path, deliberately: scripts/core_endgame.py
    // counts textual crossings and cannot tell a comment from a call, so a
    // comment naming the old path would leave the edge on the graph after the
    // code was gone. Its own Caveats section says so; this is the first time
    // that caveat has changed what got written.
